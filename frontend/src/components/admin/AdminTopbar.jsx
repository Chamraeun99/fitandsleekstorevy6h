...
export default function AdminTopbar({ sidebarOpen = true }) {
  ...
  return (
    <header
      className={`h-16 md:h-20 admin-surface border-b admin-border px-4 md:px-8 flex items-center justify-between sticky top-0 z-20 md:fixed md:top-0 md:right-0 md:z-40 ${
        sidebarOpen ? "md:left-[280px]" : "md:left-24"
      }`}
    >
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => navigate("/")}
          className="hidden sm:inline-flex items-center gap-2 h-10 px-4 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/75 dark:bg-slate-900/60 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-[rgba(var(--admin-primary-rgb),0.16)] hover:text-[var(--admin-primary)] transition"
          title="Go to site"
        >
      ...
      <div className="flex items-center gap-3 md:gap-4">
        <div className="relative w-[300px] hidden md:block" ref={searchRef}>
          ...
          <input
            ...
            className="h-11 w-full rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/75 dark:bg-slate-900/60 pl-10 pr-3 text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2"
            style={{
          ...
          {showSearch && query && (
            <div className="absolute left-0 right-0 mt-2 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 backdrop-blur-xl shadow-xl overflow-hidden">
              {results.length ? (
        ...
        <button
          ...
          className="relative h-11 w-11 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/75 dark:bg-slate-900/60 text-slate-600 dark:text-slate-300 hover:bg-[rgba(var(--admin-primary-rgb),0.16)] hover:text-[var(--admin-primary)] transition flex items-center justify-center"
        >
          {mode === "dark" ? (
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="relative h-11 w-11 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/75 dark:bg-slate-900/60 text-slate-600 dark:text-slate-300 hover:bg-[rgba(var(--admin-primary-rgb),0.16)] hover:text-[var(--admin-primary)] transition"
          >
          ...
          {showNotif && (
            <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 backdrop-blur-xl shadow-xl overflow-hidden">
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-2 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/75 dark:bg-slate-900/60 px-3 h-11 hover:bg-[rgba(var(--admin-primary-rgb),0.16)] transition"
          >
          ...
          {showProfile && (
            <div className="absolute right-0 mt-2 w-52 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 backdrop-blur-xl shadow-xl overflow-hidden">
              <button
                onClick={() => {
