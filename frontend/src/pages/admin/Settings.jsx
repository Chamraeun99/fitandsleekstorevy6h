...
export default function Settings() {
  ...
  const cardClass =
    "rounded-3xl border border-slate-200/70 dark:border-slate-700/70 bg-white/78 dark:bg-slate-900/62 backdrop-blur-xl shadow-[0_18px_40px_rgba(15,23,42,0.12)]";

  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-start gap-4">
          <span className="w-11 h-11 rounded-2xl flex items-center justify-center shadow-lg shadow-black/10" style={{ backgroundColor: accentColor }}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <aside className={`${cardClass} p-5 h-fit xl:sticky xl:top-28`}>
            ...
            <div className="mt-5 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 p-3 bg-white/60 dark:bg-slate-900/50">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Accent Preview</p>
              <div className="h-10 rounded-lg" style={{ backgroundColor: accentColor }} />
            </div>
            <button
              onClick={save}
              disabled={saving}
              className={`mt-5 w-full h-11 rounded-2xl font-semibold shadow-sm hover:shadow-md transition disabled:opacity-50 disabled:cursor-not-allowed ${accentIsWhite ? "border border-slate-300" : ""}`}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <input type="text" value={formPrimaryColor} onChange={(e) => handleThemeColorChange(e.target.value)} className={inputClass} placeholder="#F58E27" />
                  </div>
        ...
        <div className="sticky bottom-4 z-10">
          <div className="ml-auto w-full max-w-7xl">
            <div className={`${cardClass} p-3 md:p-4 flex items-center justify-between gap-3`}>
              <p className="text-sm text-slate-500 dark:text-slate-400">Remember to save after changing appearance, commerce, or legal settings.</p>
              <button
                onClick={save}
                disabled={saving}
                className={`h-11 px-6 rounded-2xl font-semibold shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2 ${accentIsWhite ? "border border-slate-300" : ""}`}
                style={{ backgroundColor: accentColor, color: accentIsWhite ? "#0b0b0f" : "#FFFFFF" }}
              >
                {saving ? (
                  <>
