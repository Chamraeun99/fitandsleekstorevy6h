  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* ── Filter Bar: Gender + Section Type ── */}
        <div className="mb-6 flex flex-wrap items-center gap-3">
          ...
          <div className="relative" ref={genderDropdownRef}>
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border font-medium text-sm transition-all duration-150 min-w-[150px] justify-between"
              style={{
            ...
            >
              ...
              <svg
                ...
                style={{ transform: genderDropdownOpen ? "rotate(180deg)" : "rotate(0deg)", opacity: 0.6 }}
            ...
            {genderDropdownOpen && (
              <div
                className="absolute left-0 mt-1 z-30 rounded-xl overflow-hidden"
                style={{
                  minWidth: "180px",
          ...
          {/* Section / Type multi-select dropdown (from Homepage Sections) */}
          {sectionTabs.length > 0 && (
            <div className="relative" ref={sectionDropdownRef}>
              ...
              >
                ...
                <svg
                  className="w-4 h-4 flex-shrink-0 transition-transform duration-200"
                  style={{ transform: sectionDropdownOpen ? "rotate(180deg)" : "rotate(0deg)", opacity: 0.6 }}
                  fill="none" stroke="currentColor" viewBox="0 0 24 24"
                >
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
          ...
          {(selectedGenders.length > 0 || selectedSections.length > 0) && (
            <div className="flex flex-wrap items-center gap-2">
              {selectedGenders.map((k) => (
                <span
                  key={k}
                  className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold"
                  style={{
                    backgroundColor: accentIsWhite ? "#f1f5f9" : accentColor + "22",
                    color: accentIsWhite ? "#334155" : accentColor,
                    border: `1px solid ${accentIsWhite ? "#e2e8f0" : accentColor + "55"}`,
                  }}
                >
                  {GENDER_OPTIONS.find((g) => g.key === k)?.label ?? k}
                  <button
                    onClick={() => setSelectedGenders((prev) => prev.filter((x) => x !== k))}
              ...
              {selectedSections.map((k) => (
                ...
                >
                  {sectionTabs.find((t) => t.key === k)?.title ?? k}
                  <button
                    onClick={() => setSelectedSections((prev) => prev.filter((x) => x !== k))}
                    className="hover:opacity-70 transition-opacity"
                  >
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />
              ...
              <button
                onClick={() => { setSelectedGenders([]); setSelectedSections([]); }}
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">{filteredRows.length} product{filteredRows.length !== 1 ? "s" : ""}</p>
                </div>
              </div>
