...
export default function AdminLayout() {
  ...
  return (
    <div className="admin-theme admin-root admin-gemini-shell min-h-screen overflow-x-hidden text-slate-800 dark:text-slate-100 font-sans flex admin-soft">
      <aside
        className={`${sidebarOpen ? "w-[280px]" : "w-24"} hidden md:block fixed left-0 top-0 h-screen admin-surface border-r admin-border transition-all duration-300 z-30 overflow-hidden`}
      >
        <div className="h-20 px-4 flex items-center justify-between border-b admin-border">
          <div className="overflow-hidden">
            <Logo className={sidebarOpen ? "h-14 w-auto" : "h-11 w-auto"} src={logoSrc} alt="Fit&Sleek" />
          </div>
          <button
            onClick={() => setSidebarOpen((prev) => !prev)}
            className="h-10 w-10 rounded-2xl border admin-border bg-white/70 dark:bg-slate-900/60 text-slate-500 dark:text-slate-300 hover:bg-[rgba(var(--admin-primary-rgb),0.18)] transition"
          >
        ...
        <nav className="p-3 h-[calc(100vh-80px)] overflow-y-auto scrollbar-hover">
            ...
            {directItems.map((item) => (
              <li key={item.path}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-4 py-3 rounded-2xl border border-transparent border-l-2 transition-all duration-200 ${
                      isActive
                        ...
                        {group.children.map((item) => (
                          <li key={item.path}>
                            <NavLink
                              to={item.path}
                              className={({ isActive }) =>
                                `flex items-center gap-3 px-4 py-2.5 rounded-2xl border border-transparent border-l-2 text-sm transition-all duration-200 ${
                                  isActive
                                    ? "text-white shadow-sm border-l-emerald-500"
          ...
          {sidebarOpen && (
            <div className="mt-5 rounded-2xl border admin-border admin-surface p-3" style={{ boxShadow: "none" }}>
